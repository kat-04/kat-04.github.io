...
void increment_frame(map<int, bool> *rules, vector<Vec4> *curAlive, vector<Vec4> *newAlive, 
    ...
    #pragma omp parallel
    {
        ...
        for (auto voxel : *curAlive) {
            ...
            if ((*rules)[27 + num_alive]) {
                // If it survives
                (*newAlive).push_back(Vec4(voxel.x, voxel.y, voxel.z, num_alive));
            } else {
            ...
            for (auto neighbor : neighbors) {
                ...
                if ((*rules)[num_alive]) {
                    ...
                    if (!is_alive(neighbor, states_tmp, n)) {
                        (*newAlive).push_back(Vec4(neighbor.x, neighbor.y, neighbor.z, num_alive));
...
void parse_input(string outputDir, string inputFile, uint64_t sideLength, map<int, bool> *rules, 
                 bool *isMoore, int *numStates, vector<Vec4> *voxels, uint8_t *states, uint8_t *states_tmp)
{
    // Open input file
    fstream input;
    ...
    while (getline(input, line)) {
        if (curLine == 0) {
            ...
            (*voxels).push_back(Vec4(x, y, z, 0));

            // Toggle state to 1
            toggle_state(Vec4(x, y, z, 0), states, n);
            toggle_state(Vec4(x, y, z, 0), states_tmp, n);
...
void write_output(string frameOutputFile, vector<Vec4> *voxels, uint8_t *states) {
...
int golOpenMPAmbient(int argc, char** argv, string outputDir) {
